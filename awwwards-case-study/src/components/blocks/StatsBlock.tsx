"use client";

import { useRef } from "react";
import { useGSAP } from "@gsap/react";
import gsap from "gsap";

import { StatsBlockProps } from "@/types/case-study";

export function StatsBlock({ stats }: StatsBlockProps) {
    const containerRef = useRef<HTMLDivElement>(null);

    useGSAP(() => {
        gsap.from(".stat-item", {
            y: 40,
            opacity: 0,
            duration: 0.8,
            stagger: 0.1,
            ease: "power3.out",
            overwrite: "auto",
            scrollTrigger: {
                trigger: containerRef.current,
                start: "top 80%",
            },
        });
    }, { scope: containerRef });

    return (
        <div ref={containerRef} className="w-full mt-[5vh] mb-[10vh] border-y border-black/10 py-20 md:py-32 will-change-transform">
            <div className="max-w-[1280px] mx-auto px-6 md:px-12">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12">
                    {stats.map((stat, i) => (
                        <div key={i} className="stat-item flex flex-col items-start text-left min-w-0 md:border-l border-text/10 md:first:border-l-0 pl-0 md:pl-12">
                            <span
                                className="w-full text-[12vw] md:text-[5vw] font-mega text-brand mb-4 block leading-none break-words"
                                style={{
                                    WebkitTextStrokeWidth: '1px',
                                    WebkitTextStrokeColor: 'currentColor',
                                }}
                            >
                                {stat.value}
                            </span>
                            <span className="font-text text-sm md:text-base text-text/50 uppercase tracking-wider break-words w-full">
                                {stat.label}
                            </span>
                        </div>
                    ))}
                </div>
            </div>
        </div>
    );
}
